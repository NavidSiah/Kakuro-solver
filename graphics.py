...
def draw_background(Grid_of_the_game):
    ...
    for i in range(height):
        for j in range(width):
            ...
            if cell_value == 'X':
                pg.draw.rect(screen, (40, 30, 20), pg.Rect(j * cells_width + 2, i * cells_height + 2, cells_width - 2, cells_height - 2))
            elif cell_value.find('\\') != -1:
                pg.draw.rect(screen, (40, 30, 20), pg.Rect(j * cells_width + 2, i * cells_height + 2, cells_width - 2, cells_height - 2))
                pg.draw.line(screen, pg.Color('white'), (j * cells_width, i * cells_height), ((j+1) * cells_width, (i+1) * cells_height), 1)
